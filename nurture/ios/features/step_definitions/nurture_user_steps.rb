...
Given(/^"([^"]*)" create a "([^"]*)" topic in the test group$/) do |user_name, topic_type|
  puts "New Nurture User '#{user_name}' created: #{$user.email}, #{$user.password}"
...
Then(/^I create another nurture user "([^"]*)" and create a topic in the test group$/) do |user_name|
  $user2 = forum_new_nurture_user(first_name: user_name).join_group
  puts GROUP_ID
  $user2.create_topic({:topic_title => "Test follow/block user", :group_id => GROUP_ID})
end

Then(/^I created another user to vote the poll$/) do
  $user2 = forum_new_nurture_user
...
Then(/^"([^"]*)" add (\d+) comment(?:s)? and "([^"]*)" added (\d+) subrepl(?:y|ies) to each comment\.$/) do |user1_name, comment_number, user2_name, subreply_number|
  puts "Nurture User #{user1_name} topic_id is #{$user.topic_id}"
  $user2 = forum_new_nurture_user(first_name: user2_name)
  ...
  comment_number.to_i.times do |comment_number|
    ...
    puts "NurtureUser #{user1_name} reply_id is #{$user.reply_id}"
    subreply_number.to_i.times do |subreply_number|
      puts "NurtureUser #{user2_name} sub reply ++; subreply number is #{subreply_number+1}"
...
Then(/^"([^"]*)" create (\d+) topic(?:s)? and (\d+) comment(?:s)? and (\d+) subrepl(?:y|ies) for each comment$/) do |user_name, arg1, comment_number, subreply_number|
  $user.create_topic
  puts "NurtureUser #{user_name} topic_id is #{$user.topic_id}, topic title is #{$user.topic_title}"
  ...
  comment_number.to_i.times do |comment_number|
    ...
    puts "NurtureUser reply_id is #{$user.reply_id}"
    subreply_number.to_i.times do |subreply_number|
      puts "NurtureUser sub reply ++"
...
Then(/^another user "([^"]*)" create (\d+) topic(?:s)? and (\d+) comment(?:s)? and (\d+) subrepl(?:y|ies) for each comment$/) do |name, arg1, comment_number, subreply_number|
  $user2 = forum_new_nurture_user(first_name: name)
  $user2.create_topic :topic_title => "Test hide/flag #{$user2.random_str}"
  puts "Nurture User #{name} topic_id is #{$user2.topic_id}, topic title is #{$user2.topic_title}"
  ...
  comment_number.to_i.times do |comment_number|
    ...
    puts "NurtureUser2 reply_id is #{$user2.reply_id}"
    subreply_number.to_i.times do |subreply_number|
      puts "NurtureUser2 sub reply ++"
...
Given(/^(\d+) other users upvote the topic and (\d+) other users downvote the topic$/) do |arg1, arg2|
  arg1.to_i.times do
    new_user = new_nurture_user.upvote_topic $user.topic_id
    puts "#{new_user.first_name} upvoted #{$user.topic_id}  >> #{$user.topic_title} << "
  end
  arg2.to_i.times do
    new_user = new_nurture_user.downvote_topic $user.topic_id
...
Given(/^(\d+) other users upvote the comment and (\d+) other users downvote the comment$/) do |arg1, arg2|
  arg1.to_i.times do
    new_user = new_nurture_user.upvote_comment $user.topic_id, $first_comment_id
    puts "#{new_user.first_name} upvoted comment #{$first_comment_id} under #{$user.topic_id}  >> #{$user.topic_title} << "
  end
  arg2.to_i.times do
    new_user = new_nurture_user.downvote_comment $user.topic_id, $first_comment_id
...
Given(/^(\d+) other users reported the topic$/) do |arg1|
  ...
  arg1.to_i.times do
    new_user = new_nurture_user.report_topic $user.topic_id, reason_poll.sample
...
Given(/^(\d+) other users reported the comment$/) do |arg1|
  ...
  arg1.to_i.times do
    new_user = new_nurture_user.report_comment $user.topic_id, $first_comment_id, reason_poll.sample
    puts "#{new_user.first_name} reported #{$user.topic_id}"
  end
end

Given(/^I create a new nurture user with name "(.*?)"$/) do |name|
  $user = forum_new_nurture_user(first_name: name)
  puts $user.email, $user.password
...
Then(/^"(.*?)" reply to (\d+) topics created by others$/) do |name, number|
  number.to_i.times do
    new_user = new_nurture_user.create_topic
