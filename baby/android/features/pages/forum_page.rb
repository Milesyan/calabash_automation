...
#------------create topics-----------------
  ...
  def create_a_group
    ...
    if element_exists "* id:'create_cancel'"
...
#-----------------
  ...
  def check_search_result_deleted(string)
    sleep 0.5
    wait_touch "* marked:'#{string}' index:1"
    wait_for_elements_exist("* {text CONTAINS 'Topic does not exist!'}", :retry_frequency => 0.05, :timeout => 3)
    sleep 1
  end
  ...
  def go_to_group_page_under_settings
    ...
    touch "* marked:'My Groups'" if element_exists "* marked:'My Groups'"
  end
