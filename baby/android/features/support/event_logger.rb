...
module BabyAndroid
  BABY_EVENTS_URL = "http://titan-appevents.glowing.com"

  class EventLogger
    include HTTParty
    extend BabyHelper

    base_uri BABY_EVENTS_URL
    ...
    def pull_log(args)
      data = {
        ...
        "raw_device_id": "ad32a9977238fa1f"
        #"email": args[:email] || args[:user].email
      }.to_param

      puts yellow("#{BABY_EVENTS_URL}/noah?#{data}")

      @res = self.class.get "/noah?#{data}"
      @actual = @res["events"]
      self
    end
